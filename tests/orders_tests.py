...
def get_order(user, order_id, expected_status=200):
    ...
    response = requests.get(
        f"http://localhost:5003/orders/{order_id}", cookies=cookie, timeout=10
    )
...
def get_user_orders(user, user_id, expected_status=200):
    ...
    cookie = {"access_token": user.token}
    response = requests.get(
        f"http://localhost:5003/orders/user/{user_id}", cookies=cookie, timeout=10
    )
...
def update_order_status(user, order_id, status, expected_status=200):
    ...
    response = requests.put(
        f"http://localhost:5003/orders/{order_id}",
        headers={"Content-Type": "application/json"},
        cookies=cookie,
        json=data,
        timeout=10,
